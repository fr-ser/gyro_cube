...
def get_gyro_log(db: Session, timestamp: int):
    return db.query(models.GyroLog).filter(models.GyroLog.timestamp == timestamp).first()
...
def create_gyro_log(db: Session, gyro_log: schemas.GyroLog):
    most_recent_ts = db.query(func.max(models.GyroSide.timestamp)).filter(
        models.GyroSide.timestamp > gyro_log.timestamp).scalar()
    db_gyro_log = models.GyroLog(**gyro_log.dict(), side_timestamp=most_recent_ts)
...
def get_gyro_side_name(db: Session, timestamp: int):
    return db.query(models.GyroSide).filter(models.GyroSide.timestamp == timestamp).first()
